...
discriminator_variables_dict = {
    "W_1": tf.Variable(tf.truncated_normal([4, 4, 512, 1024], stddev=0.002), name='Discriminator/W_1'),
    "b_1": tf.Variable(tf.constant(0.0, shape=[1024]), name='Discriminator/b_1'),
    'beta_1': tf.Variable(tf.constant(0.0, shape=[1024]), name='Discriminator/beta_1'),
    'gamma_1': tf.Variable(tf.random_normal(shape=[1024], mean=1.0, stddev=0.02), name='Discriminator/gamma_1'),

    "W_2": tf.Variable(tf.truncated_normal([3*3*1024,1], stddev=0.002), name='Discriminator/W_2'),
    "b_2": tf.Variable(tf.constant(0.0, shape=[1]), name='Discriminator/b_2'),
...
def discriminator(in_data):
    with tf.variable_scope("Discriminator"):
        ...
        re = tf.reshape(out_1, [-1, 3*3*1024])

        out_2 = tf.matmul(re, discriminator_variables_dict['W_2']) + discriminator_variables_dict['b_2']

        return (tf.sigmoid(out_2))
...
generator_variables_dict = {
    "W_1": tf.Variable(tf.truncated_normal([in_channel, 1024*s_h2*s_w2], stddev=0.02), name='Generator/W_1'),
    "b_1": tf.Variable(tf.constant(0.0, shape=[1024*s_w2*s_h2]), name='Generator/b_1'),
...
def generator(z):
    with tf.variable_scope("Generator"):
        ...
        out_1 = tf.reshape(out_1, [-1, s_h2, s_w2, 1024])
