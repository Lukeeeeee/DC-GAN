...
def generate(z, h, w, ch, is_training, reuse, batch_size=64):
    with tf.variable_scope('generator') as scope:
        ...
        z = tf.reshape(z, shape=[-1, 100], name='RESHAPE')
        fc0 = lay.fully_connect_layer(z, 'g_fc0_lin', h2 * w2 * 512)
        fc0 = tf.reshape(fc0, [-1, h2, w2, 512])
        ...
        decon3 = lay.deconv_2d_layer(fc0, 'g_decon3', [5, 5, 128, 512], [batch_size, h1, w1, 128],
                                     strides=[1, 2, 2, 1])
        decon3 = lay.batch_norm_official(decon3, is_training=is_training, reuse=reuse, name='g_bn3')
        decon3 = tf.nn.relu(decon3)

        decon4 = lay.deconv_2d_layer(decon3, 'g_decon4', [5, 5, ch, 128], [batch_size, h, w, 16],
                                     strides=[1, 2, 2, 1])
...
def decrim(x, is_training, reuse, batch_size=64):
    with tf.variable_scope('decriminator') as scope:
        conv0 = lay.conv_2d_layer(x, 'd_conv0', [5, 5, 16, 32], strides=[1, 2, 2, 1])
        conv0 = lay.batch_norm_official(conv0, is_training=is_training, reuse=reuse, name='d_bn0')
        conv0 = lay.leaky_relu(conv0)

        conv1 = lay.conv_2d_layer(conv0, 'd_conv1', [5, 5, 32, 64], strides=[1, 2, 2, 1])
        ...
        conv4_flatten = tf.reshape(conv1, [-1, 64 * 2 * 2])
